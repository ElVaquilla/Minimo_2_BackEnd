...
public class GameManagerImpl implements GameManager {
    ...
    protected List<Denuncia> denuncias;
    ...
    public DenunciaRespuesta denunciarJugador(Denuncia denuncia)throws FaltanDatosException, UserNotFoundException{
        DenunciaRespuesta respuesta = new DenunciaRespuesta();
        if(denuncia.getDate() == null || denuncia.getMessage() == null || denuncia.getSender() == null || denuncia.getTitle() == null) {
            logger.warn("Faltan datos en la denuncia");
            throw new FaltanDatosException();
        }
        boolean encontrado = false;
        for(Jugador j : this.findAllJugadores()){
            if(j.getUsername().equals(denuncia.getSender()))
                encontrado = true;
        }
        if(!encontrado) {
            logger.warn("El jugador no existe");
            throw new UserNotFoundException();
        }
        else{
            logger.info("La denuncia se ha guardado correctamente");
