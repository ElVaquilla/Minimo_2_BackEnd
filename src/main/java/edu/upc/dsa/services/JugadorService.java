...
public class JugadorService {
    ...
    @POST
    @ApiOperation(value = "Denuncia a jugador")
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "Succesful", response = DenunciaRespuesta.class),
            @ApiResponse(code = 404, message = "Not found", response = DenunciaRespuesta.class),
            @ApiResponse(code = 501, message = "Faltan datos", response = DenunciaRespuesta.class)

    })
    @Path("/issue")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response denunciaJugador(Denuncia denuncia) throws UserNotFoundException, FaltanDatosException {
        DenunciaRespuesta r = new DenunciaRespuesta();
        try {
            r = this.gm.denunciarJugador(denuncia);
            return Response.status(201).entity(r).build();
        }
        catch(UserNotFoundException e){
            r.setMessage(e.getMessage());
            ...
            r.setMessage(e.getMessage());
            return Response.status(501).entity(r).build();
        }
    }
